...
# Función auxiliar para procesar y guardar la imagen
def guardar_imagen(file_storage):
    if file_storage and '.' in file_storage.filename:
        ext = file_storage.filename.rsplit('.', 1)[1].lower()
        if ext in ALLOWED_EXTENSIONS:
            nombre_limpio = secure_filename(file_storage.filename)
...
def nuevo_producto_fisico():
    if request.method == "POST":
        try:
            imagen_nombre = guardar_imagen(request.files.get("imagen"))
            ...
            flash("Revisa que los campos numéricos sean correctos.", "danger")
        except Exception:
            db.session.rollback()
            flash("Ocurrió un error. Verifica que el código no esté duplicado.", "danger")
    ...
    if request.method == "POST":
        try:
            ...
            if file and file.filename != '':
                producto.imagen_url = guardar_imagen(file)
